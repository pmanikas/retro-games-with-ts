...
class Game {
    ...
    clickHandler = (e) => {
        ...
        if(e.target.hasAttribute('data-button-save-options')) this.saveOptionsHandler();
    ...
    handleLose = (ballCollisionDirection) => {
        ...
        this.botPlayer.resetPaddle();
        this.ball.init();
        if (this.humanPlayer.score >= Number(this.setGameScore) || this.botPlayer.score >= Number(this.setGameScore)) this.handleFinish();
    }
    ...
    update = (time) => {
        if (this.lastTime !== null && this.gameStatus === "STARTED") {
            ...
            this.ball.update(
                delta, 
                [this.humanPlayer.paddle.rect, this.botPlayer.paddle.rect]
            );
